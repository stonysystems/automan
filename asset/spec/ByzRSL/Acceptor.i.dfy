...
// predicate Received2avsAreValid(s:LAcceptor, opn:OperationNumber)
//     // requires HasReceived2avOfOpn(s.received_2avs, opn)
// {
//     if opn in s.received_2avs then
//         && (forall src :: src in s.received_2avs[opn].received_2av_message_senders ==> src in s.constants.all.config.replica_ids)
//         && (|s.received_2avs[opn].received_2av_message_senders| == |s.received_2avs[opn].candidate_value|)
//         && (|s.received_2avs[opn].received_2av_message_senders| <= |s.constants.all.config.replica_ids|)
//     else
//         true
// }
...
predicate Received2avSetCorrect(r2avs:AcceptorTuple, bal:Ballot, opn:OperationNumber, c:LConfiguration)
{
    ...
    && (forall i,j :: 0 <= i < |r2avs.received_2av_packets| && 0 <= j < |r2avs.received_2av_packets| && i != j 
                    ==> r2avs.received_2av_packets[i] != r2avs.received_2av_packets[j] 
                    && r2avs.received_2av_packets[i].src != r2avs.received_2av_packets[j].src
                    )
...
{
    // && |ls| <= |c.replica_ids|
    && (forall opn :: opn in r2avs ==> Received2avSetCorrect(r2avs[opn], bal, opn, c))
}
    ...
    requires inp.msg.RslMessage_1a?
{
    ...
    if inp.src in s.constants.all.config.replica_ids && BalLt(s.max_bal, m.bal_1a) && LReplicaConstantsValid(s.constants) then
        ...
        //                             RslMessage_1b(m.bal_1a, s.log_truncation_point, s.votes, s.sent_2bs)) ]
...
predicate LAcceptorProcess1c(s:LAcceptor, s':LAcceptor, inp:RslPacket, sent_packets:seq<RslPacket>)
    ...
    // requires forall m :: m in inp.msg.msgs_1b ==> m.RslMessage_1b?
...
{ 
    ...
    // if 
    //     // && (m.opn_1c in s.votes && s.votes[m.opn_1c].max_value_bal < m.bal_1c) //这个需要吗？
    //     && (forall m_1b :: m_1b in m.msgs_1b ==> m_1b.RslMessage_1b?)
    //     && 
    //     ( || (forall m_1b :: m_1b in m.msgs_1b ==> m.opn_1c !in m_1b.votes)
    //       || LValIsSafeAt(m.val_1c, /*m.bal_1c,*/ m.msgs_1b, m.opn_1c, byzq, wq)) then
        ...
        // && s'.sent_2bs == s.sent_2bs
        // && s'.received_2avs == s.received_2avs
        // && s'.opn_to_be_send_2b == s.opn_to_be_send_2b
...
predicate LAcceptorProcess2av(s:LAcceptor, s':LAcceptor, inp:RslPacket)
    ...
    // requires inp.src in s.constants.all.config.replica_ids
    // requires AcceptorStateCorrect(s.received_2avs, s.max_bal, s.constants.all.config)
            // && (forall src :: src in s.received_2avs[inp.msg.opn_2av].received_2av_message_senders ==> src in s.constants.all.config.replica_ids)
            // && |src|
...
// predicate LAcceptorDecide2bVal(s:LAcceptor, s':LAcceptor)
...
//     // var v := [];
//     // && s' == s.(val_to_be_sent_2b := ValToBeSent2bKnown(v))
//     exists v ::
//         v in s.received_2avs[opn].candidate_value 
//         && CountMatchesInSeq(s.received_2avs[opn].candidate_value, x => x == v) >= quorum
//         // && LBroadcastToEveryone(s.constants.all.config, s.constants.my_index, RslMessage_2b(s.max_bal, opn, v), sent_packets)
//         && s' == s.(val_to_be_sent_2b := ValToBeSent2bKnown(v))
// }
...
// predicate LAcceptorMaybeSent2b(s:LAcceptor, s':LAcceptor, sent_packets:seq<RslPacket>)
//     requires s.val_to_be_sent_2b.ValToBeSent2bKnown?
//     // requires s.opn_to_be_send_2b in s.received_2avs
//     // requires |s.received_2avs[s.opn_to_be_send_2b].received_2av_message_senders| >= LByzQuorumSize(s.constants.all.config)
//     // requires IsByzQuorumSendSame2av(s.received_2avs[s.opn_to_be_send_2b].candidate_value, LByzQuorumSize(s.constants.all.config)) 
...
//         && CountMatchesInSeq(s.received_2avs[opn].candidate_value, x => x == v) >= quorum
//         && LBroadcastToEveryone(s.constants.all.config, s.constants.my_index, RslMessage_2b(s.max_bal, opn, v), sent_packets)
//         && s' == s.(sent_2bs := s.sent_2bs[opn := Sent2b(s.max_bal, v)],
//                     // received_2avs := RemoveElt(s.received_2avs, opn),
//                     opn_to_be_send_2b := s.opn_to_be_send_2b + 1)

//     // var opn := s.opn_to_be_send_2b;
//     // var quorum := LByzQuorumSize(s.constants.all.config);
//     // if && opn in s.received_2avs
//     //    && |s.received_2avs[opn].received_2av_message_senders| >= quorum
//     //    && IsByzQuorumSendSame2av(s.received_2avs[opn].candidate_value, quorum) then
//     //     exists v ::
//     //         v in s.received_2avs[opn].candidate_value 
//     //         && CountMatchesInSeq(s.received_2avs[opn].candidate_value, x => x == v) >= quorum
//     //         && LBroadcastToEveryone(s.constants.all.config, s.constants.my_index, RslMessage_2b(s.max_bal, opn, v), sent_packets)
...
predicate LAcceptorTruncateLog(s:LAcceptor, s':LAcceptor, opn:OperationNumber)
{
    if opn <= s.log_truncation_point then
        ...
        // && RemoveSent2bsBeforeLogTruncationPoint(s.sent_2bs, s'.sent_2bs, opn)
}
}