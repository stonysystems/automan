...
predicate CheckRequestValidInReqSeq(s:seq<Request>, r:Request)
{
  if |s| == 0 then
    ...
    var v := if RequestsMatch(s[0], r) && s[0].request != r.request then false else true;
    if v then
      CheckRequestValidInReqSeq(s[1..], r) 
    else
      v
}

// predicate RequestMatchesClientReq(s:seq<Request>, r:Request)
//   requires exists r1 :: r1 in s && RequestsMatch(r1, r)
// {
//   var r1 :| r1 in s && RequestsMatch(r1, r);
//   r1.request == r.request
// }

predicate CheckRequestValid(s:ElectionState, r:Request)
{
  // if exists r1 :: r1 in s.requests_received_this_epoch && RequestsMatch(r1, r) then
  //   RequestMatchesClientReq(s.requests_received_this_epoch, r)
  // else if exists r1 :: r1 in s.requests_received_prev_epochs && RequestsMatch(r1, r) then
  //   RequestMatchesClientReq(s.requests_received_prev_epochs, r)
  // else
  //   true
  && CheckRequestValidInReqSeq(s.requests_received_this_epoch, r)
  && CheckRequestValidInReqSeq(s.requests_received_prev_epochs, r)
}
