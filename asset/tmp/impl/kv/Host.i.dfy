...
module Impl_SHT__Host_i 
{
	...
	CHost(
		...
		numDelegations: int, 
		ghost receivedRequests: seq<CAppRequest>
	)
	...
	predicate CHostIsValid(s: CHost) 
	{
		...
		&& 
		(forall i :: i in s.receivedRequests ==> CAppRequestIsValid(i))
	}
	...
	predicate CHostIsAbstractable(s: CHost) 
	{
		...
		&& 
		(forall i :: i in s.receivedRequests ==> CAppRequestIsAbstractable(i))
	}

	function AbstractifyCHostToHost(s: CHost) : Host 
		requires CHostIsAbstractable(s)
	{
		Host(AbstractifyCConstantsToConstants(s.constants), AbstractifyEndPointToNodeIdentity(s.me), AbstractifyCDelegationMapToDelegationMap(s.delegationMap), AbstractifyHashtableToHashtable(s.h), AbstractifyCSingleDeliveryAcctToSingleDeliveryAcct(s.sd), , s.numDelegations, AbstractifySeq(s.receivedRequests, AbstractifyCAppRequestToAppRequest))
	}
	...
	function method CValidOptionalValue(opt: COptionalValue) : bool 
	...
	function method CHashtableLookup(h: Hashtable, k: Key) : COptionalValue 
		requires HashtableIsValid(h)
		requires KeyIsValid(k)
		ensures var lr := HashtableLookup(AbstractifyHashtableToHashtable(h), AbstractifyKeyToKey(k)); var cr := CHashtableLookup(h, k); COptionalValueIsValid(cr) && (AbstractifyCOptionalValueToOptionalValue(cr)) == (lr)
	{
		if k in h then 
			CValuePresent(h[k]) 
		else 
					...
					var t1 := 
						...
						var t1 := 
							...
							var t1 := 
								...
								var t2 := 
									if CValidKey(k) && shouldSend then 
										var t1 := 
											msg; 										
										var t2 := 
											s.(sd := t1.2, receivedRequests := newReceivedRequests, receivedPacket := None); 										
										var t3 := 
										...
										var t2 := 
											if shouldSend && CValidKey(k) && CValidOptionalValue(ov) then 
												var t1 := 
													msg; 												
												var t2 := 
													s.(h := newhtable, sd := t1.2, receivedRequests := reqs, receivedPacket := None); 												
												var t3 := 
													{CPacket(src, s.me, t1.1)}; 												
												(t2, t1, t3) 
											else 
					...
					if (pkt.msg.m.recipient == s.me) || ((!CValidKeyRange(pkt.msg.m.kr)) || ((CEmptyKeyRange(pkt.msg.m.kr)) || ((pkt.msg.m.recipient !in s.constants.hostIds) || ((!CDelegateForKeyRangeIsHost(s.delegationMap, pkt.msg.m.kr, s.me)) || (|CExtractRange(s.h, pkt.msg.m.kr)| >= Cmax_hashtable_size()))))) then 
						...
						var t2 := 
							if shouldSend then 
								...
								var t2 := 
									s.(h := CBulkRemoveHashtable(s.h, kr), delegationMap := CUpdateDelegationMap(s.delegationMap, kr, recipient), sd := t1.2, numDelegations := s.numDelegations + 1, receivedPacket := None); 								
								(t2, t1) 
							else 
								var t1 := 
									{}; 								
