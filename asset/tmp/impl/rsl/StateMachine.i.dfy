...
module Impl_LiveRSL__StateMachine_i 
{
	...
	{
		var [Printer for Prog.pattern_t.PatCtor hasn't been implemented.] := 
	...
	function method CHandleRequestBatchHidden(state: CAppState, batch: CRequestBatch) : (seq<CAppState>, seq<CReply>) 
		...
		ensures var [Printer for Prog.pattern_t.PatCtor hasn't been implemented.] := CHandleRequestBatchHidden(state, batch); |states| == |batch| + 1 && |replies| == |batch| && (forall i :: 0 <= i && i < |batch| ==> replies[i].CReply?)
		decreases |batch|
	...
	function method CHandleRequestBatch(state: CAppState, batch: CRequestBatch) : (seq<CAppState>, seq<CReply>) 
		requires CAppStateIsValid(state)
		requires CRequestBatchIsValid(batch)
		ensures var [Printer for Prog.pattern_t.PatCtor hasn't been implemented.] := CHandleRequestBatch(state, batch); states[0] == state && |states| == |batch| + 1 && |replies| == |batch| && (forall i :: 0 <= i && i < |batch| ==> replies[i].CReply? && (states[i + 1], replies[i].reply) == CAppHandleRequest(states[i], batch[i].request) && replies[i].client == batch[i].client && replies[i].seqno == batch[i].seqno)
