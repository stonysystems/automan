...
module Impl_LiveRSL__Acceptor_i 
{
	...
	{
		var t1 := (forall opn | opn >= log_truncation_point && (opn in votes || opn == new_opn) :: if opn == new_opn then new_vote else votes[opn]); 		
		t1
	}
	...
	{
		var t1 := var m := inp.msg; var t1 := if inp.src in s.constants.all.config.replica_ids && CBalLt(s.max_bal, m.bal_1a) && CReplicaConstantsValid(s.constants) then var t1 := [CPacket(inp.src, s.constants.all.config.replica_ids[s.constants.my_index], CMessage_1b(m.bal_1a, s.log_truncation_point, s.votes))]; var t2 := s.(max_bal := m.bal_1a); (t2, t1) else var t1 := s; var t2 := []; (t1, t2); (t1.1, t1.0); 		
		(t1.1, t1.0)
	}
	...
	{
		var t1 := var m := inp.msg; var t1 := var newLogTruncationPoint := if inp.msg.opn_2a - s.constants.all.params.max_log_length + 1 > s.log_truncation_point then inp.msg.opn_2a - s.constants.all.params.max_log_length + 1 else s.log_truncation_point; var t1 := CBroadcastToEveryone(s.constants.all.config, s.constants.my_index, CMessage_2b(m.bal_2a, m.opn_2a, m.val_2a)); var t2 := m.bal_2a; var t3 := newLogTruncationPoint; var t4 := if s.log_truncation_point <= inp.msg.opn_2a then var t1 := CAddVoteAndRemoveOldOnes(s.votes, m.opn_2a, CVote(m.bal_2a, m.val_2a), newLogTruncationPoint); t1 else var t1 := s.votes; t1; var t5 := s.constants; var t6 := s.last_checkpointed_operation; (CAcceptor(t5, t2, t4, t6, t3), t1); (t1.1, t1.0); 		
		(t1.1, t1.0)
	}
	...
	{
		var t1 := if inp.src in s.constants.all.config.replica_ids then var t1 := var sender_index := CGetReplicaIndex(inp.src, s.constants.all.config); var t1 := if 0 <= sender_index && sender_index < |s.last_checkpointed_operation| && inp.msg.opn_ckpt > s.last_checkpointed_operation[sender_index] then var t1 := s.last_checkpointed_operation[sender_index := inp.msg.opn_ckpt]; var t2 := s.constants; var t3 := s.max_bal; var t4 := s.votes; var t5 := s.log_truncation_point; CAcceptor(t2, t3, t4, t1, t5) else var t1 := s; t1; t1; t1 else var t1 := s; t1; 		
		t1
	}
	...
	{
		var t1 := var t := seq(10, (idx) => 1); var t1 := if opn <= s.log_truncation_point then var t1 := s; t1 else var t1 := CRemoveVotesBeforeLogTruncationPoint(s.votes, opn); var t2 := s.(log_truncation_point := opn, votes := t1); t2; t1; 		
		t1
	}
}
