...
module Impl_LiveRSL__Election_i 
{
	...
	function method CRequestsMatch(r1: CRequest, r2: CRequest) : bool 
		...
		ensures var  := RequestsMatch(AbstractifyCRequestToRequest(r1), AbstractifyCRequestToRequest(r2)); var  := CRequestsMatch(r1, r2); () == ()
	{
		r1.CRequest? 
		&& 
		r2.CRequest? 
	...
	function method CRequestSatisfiedBy(r1: CRequest, r2: CRequest) : bool 
		requires CRequestIsValid(r1)
		requires CRequestIsValid(r2)
		ensures var  := RequestSatisfiedBy(AbstractifyCRequestToRequest(r1), AbstractifyCRequestToRequest(r2)); var  := CRequestSatisfiedBy(r1, r2); () == ()
	{
		r1.CRequest? 
		&& 
		r2.CRequest? 
								...
								if CBalLt(es.current_view, p.msg.bal_heartbeat) then 
									var t1 := 
										var new_epoch_length := 
											CUpperBoundedAddition(es.epoch_length, es.epoch_length, es.constants.all.params.max_integer_val); 										
										var t1 := 
											es.(current_view := p.msg.bal_heartbeat, current_view_suspectors := if p.msg.suspicious then {sender_index} else {}, epoch_length := new_epoch_length, epoch_end_time := CUpperBoundedAddition(clock, new_epoch_length, es.constants.all.params.max_integer_val), requests_received_prev_epochs := CBoundRequestSequence(es.requests_received_prev_epochs + es.requests_received_this_epoch, es.constants.all.params.max_integer_val), requests_received_this_epoch := []); 										
										t1; 									
									t1 
								else 
									var t1 := 
				...
				var t1 := 
					if |es.requests_received_prev_epochs| == 0 then 
						var t1 := 
							...
							var t1 := 
								es.(epoch_length := new_epoch_length, epoch_end_time := CUpperBoundedAddition(clock, new_epoch_length, es.constants.all.params.max_integer_val), requests_received_prev_epochs := es.requests_received_this_epoch, requests_received_this_epoch := []); 							
							t1; 						
						t1 
					else 
						var t1 := 
							es.(current_view_suspectors := es.current_view_suspectors + {es.constants.my_index}, epoch_end_time := CUpperBoundedAddition(clock, es.epoch_length, es.constants.all.params.max_integer_val), requests_received_prev_epochs := CBoundRequestSequence(es.requests_received_prev_epochs + es.requests_received_this_epoch, es.constants.all.params.max_integer_val), requests_received_this_epoch := []); 						
						t1; 				
				t1; 		
		t1
	}
			...
			if |es.current_view_suspectors| < CMinQuorumSize(es.constants.all.config) || !CtUpperBound(es.current_view.seqno, es.constants.all.params.max_integer_val) then 
				...
				var t1 := 
					var new_epoch_length := 
						CUpperBoundedAddition(es.epoch_length, es.epoch_length, es.constants.all.params.max_integer_val); 					
					var t1 := 
						es.(current_view := CComputeSuccessorView(es.current_view, es.constants.all), current_view_suspectors := {}, epoch_length := new_epoch_length, epoch_end_time := CUpperBoundedAddition(clock, new_epoch_length, es.constants.all.params.max_integer_val), requests_received_prev_epochs := CBoundRequestSequence(es.requests_received_prev_epochs + es.requests_received_this_epoch, es.constants.all.params.max_integer_val), requests_received_this_epoch := []); 					
					t1; 				
				t1; 		
		t1
	}
