...
lemma TruncateAckPreservesUnAckListProperties(unAcked:seq<SingleMessage>, old_seqno:nat, new_seqno:nat, id:NodeIdentity)
...
lemma TruncateAckPreservesSubset(unAcked:seq<SingleMessage>)
...
lemma UnAckedListFinalEntry(unAcked:seq<SingleMessage>, old_seqno:nat)
    requires forall i :: 0 <= i < |unAcked| ==> unAcked[i].SingleMessage?
    requires forall i, j :: 0 <= i && j == i + 1 && j < |unAcked|
                ==> unAcked[i].seqno + 1 == unAcked[j].seqno
    requires |unAcked| > 0 ==> unAcked[0].seqno == old_seqno + 1;
...
lemma SendSingleMessagePreservesUnAckListProperties(s:SingleDeliveryAcct, s':SingleDeliveryAcct, m:Message, sm:SingleMessage, params:Parameters, shouldSend:bool)
            ...
            if UniqueInFlightPacketClaimsKey(s, k) {
                ...
                if pkt != rpkt {
                    ...
                    if NewSingleMessage(s.hosts[id].sd, pkt) {
                        ...
                        assert h'.sd == h.sd.(receiveState := h.sd.receiveState[pkt.src := (last_seqno + 1)]);
    ...
    ensures EachKeyClaimedInExactlyOnePlace(s');
{
    ...
    // assert rpkt.msg.SingleMessage? ==> ReceiveRealPacket(h.sd, h'.sd, rpkt) && ShouldAckSingleMessage(h.sd,rpkt) ==> var m := Ack(rpkt.msg.seqno); ack == Packet(dst:=rpkt.src, src:=rpkt.dst, msg:=m);
    assert ReceiveSingleMessage(h.sd, h'.sd, rpkt, ack, out);
    // assert ;
...
lemma PacketStillInFlightForward_ReceiveSingle(s:SHT_State, s':SHT_State, id:NodeIdentity, out:set<Packet>, rpkt:Packet, p:Packet, ack:Packet)
    ...
    // requires ShouldAckSingleMessage(s.hosts[id].sd,rpkt) ==> var m := Ack(rpkt.msg.seqno); ack == Packet(dst:=rpkt.src, src:=rpkt.dst, msg:=m);
    // requires ReceiveSingleMessage(s.hosts[id].sd, s'.hosts[id].sd, rpkt, ack, out);
    requires NewSingleMessage(s.hosts[id].sd, rpkt);
    requires p!=rpkt;
...
lemma NextInv_NextShard_EachKeyClaimedInExactlyOnePlace(s:SHT_State, s':SHT_State, id:NodeIdentity, recv:set<Packet>, out:set<Packet>, kr:KeyRange, recipient:NodeIdentity, sm:SingleMessage, shouldSend:bool)
    requires Inv(s);
//    requires PacketsHaveSenderUniqueSeqnos(s');
    requires MapComplete(s');
    requires PacketsHaveSaneHeaders(s');
...
lemma NextInv_Shard(s:SHT_State, s':SHT_State, id:NodeIdentity, recv:set<Packet>, out:set<Packet>, kr:KeyRange, recipient:NodeIdentity, sm:SingleMessage, shouldSend:bool)
    requires Inv(s);
    requires MapComplete(s');
    requires SHT_Next(s, s');
    requires SHT_NextPred(s, s', id, recv, out);
