...
function method CDelegateForKey(m:CDelegationMap, k:Key) : (EndPoint)
    requires 0<|m.lows|;
     requires CDelegationMapIsValid(m);
     ensures var ep := CDelegateForKey(m,k);
                EndPointIsValid(ep)
...
method {:induction false} {:timeLimitMultiplier 4} CUpdateDelegationMap(m:CDelegationMap, newkr:KeyRange, id:EndPoint) returns (ok:bool, m':CDelegationMap)
    requires CDelegationMapIsValid(m);
    requires EndPointIsValid(id);
    requires !EmptyKeyRange(newkr);
