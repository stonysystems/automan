...
/************************** AutoMan Translation ************************/
	...
	{
		var constants := CConstants(rootIdentity, hostIds, params);
		var delegationMap := CDelegationMap([Mapping(KeyZero(), constants.rootIdentity)]); /** Manually Added for I1 */
		var t1 := 
			CHost(
                CConstants(rootIdentity, hostIds, params), 
                id, 
                // CDelegationMap_Init(rootIdentity), 
                delegationMap, /** Manually Added */
                map[], 
							...
							// var t1 := 
								...
								// var t2 := 
									if ValidKey(k) && t1.2 /** Manually Added */ then 
										(s.(sd := t1.0, receivedRequests := newReceivedRequests, receivedPacket := None), t1.1, msg, [CPacket(src, s.me, t1.1)], t1.2)
										// var t1_1 := 
										// 	msg; 										
										// var t2 := 
										...
										(s.(receivedPacket := None), t1.1, msg, [], t1.2)
										// var t1 := 
									...
									// var t1 := 
										...
										// var t2 := 
											if t1.2 /** Manually Added */ && ValidKey(k) && CValidOptionalValue(ov) then 
												(s.(h := newhtable, sd := t1.0, receivedRequests := reqs, receivedPacket := None), t1.1, msg, [CPacket(src, s.me, t1.1)], t1.2)
												// var t1_1 := 
												// 	msg; 												
												// var t2 := 
    ...
    method CNextDelegateReal(s: CHost, pkt: CPacket) returns (s':CHost, out:seq<CPacket>)
		requires CHostIsValid(s)
		requires CPacketIsValid(pkt)
		requires pkt.msg.CSingleMessage?
		requires pkt.msg.m.CDelegate?
		requires !EmptyKeyRange(pkt.msg.m.range)
        // ensures var (s', out) := CNextDelegateReal(s,pkt);
        ensures CHostIsValid(s')
	...
	{
		// var t1 := 
			if pkt.src in s.constants.hostIds { 
				var ok, delegationMap' := 
					CUpdateDelegationMap(s.delegationMap, pkt.msg.m.range, s.me); 				
				var h' := 
					CBulkUpdateHashtable(s.h, pkt.msg.m.range, pkt.msg.m.h); 
				s' := s.(delegationMap := delegationMap', h := h', numDelegations := s.numDelegations + 1, receivedPacket := None);	/** Manually Added for I1 */			
				// var t3 := 
				// 	s.numDelegations + 1; 				
				// (t1, t2, t3) 
			} else {
				s' := s.(receivedPacket := None); /** Manually Added for I1 */
				// var t1 := 
				// 	s.delegationMap; 				
				// var t2 := 
				// 	s.h; 				
		...
		// (t3, t2)
		lemma_CNextDelegateReal(s,pkt,s',out);
	}

	lemma {:axiom} lemma_CNextDelegateReal(s:CHost, pkt:CPacket, s':CHost, out:seq<CPacket>)
    requires CHostIsValid(s)
    requires CPacketIsValid(pkt)
    requires pkt.msg.CSingleMessage?;
    requires pkt.msg.m.CDelegate?
    ...
    method CNextShardReal(s: CHost, pkt: CPacket) returns (s':CHost, sm:CSingleMessage, out:seq<CPacket>, shouldSend:bool)
		requires CHostIsValid(s)
		requires CPacketIsValid(pkt)
		requires pkt.msg.CSingleMessage?
		...
		// ensures var (s', out, sm, shouldSend) := CNextShardReal(s, pkt); CHostIsValid(s') && (forall i :: i in out ==> CPacketIsValid(i)) && CSingleMessageIsValid(sm) && NextShardReal(AbstractifyCHostToHost(s), AbstractifyCHostToHost(s'), AbstractifyCPacketToPacket(pkt), AbstractifySet(out, AbstractifyCPacketToPacket), AbstractifyCSingleMessageToSingleMessage(sm), shouldSend)
	{
		// var t1 := 
			...
			// var t1 := 
				var recipient := 
					pkt.msg.m.recipient; 
				var b := DelegateForKeyRangeIsHostImpl(s.delegationMap, kr, s.me);				
				// var t1 := 
					if (pkt.msg.m.recipient == s.me) || ((!CValidKeyRange(pkt.msg.m.kr)) || ((EmptyKeyRange(pkt.msg.m.kr)) || ((pkt.msg.m.recipient !in s.constants.hostIds) || ((!b) || (|CExtractRange(s.h, pkt.msg.m.kr)| >= max_hashtable_size()))))) { 
						sm := CInvalidMessage(); 						
						shouldSend := true; 						
						...
						// var t2 := 
						if t1.2 /** Manually Modified */ { 
							var ok, delegationMap' := CUpdateDelegationMap(s.delegationMap, kr, recipient);
							out := [CPacket(recipient, s.me, t1.1)]; 								
							s' := s.(h := CBulkRemoveHashtable(s.h, kr), delegationMap := delegationMap', sd := t1.0, numDelegations := s.numDelegations + 1, receivedPacket := None); 								
							// (t2, t1_1) 
						} else { 
							out := []; 								
		...
		// 	(t1.3, t1.2, t1.1, t1.0); 		
		// (t1.3, t1.2, t1.1, t1.0)
		lemma_CNextShardReal(s,pkt,s',sm,out,shouldSend);
	}

	lemma {:axiom} lemma_CNextShardReal(s:CHost, pkt:CPacket, s':CHost, sm:CSingleMessage, out:seq<CPacket>, shouldSend:bool)
		requires pkt.msg.CSingleMessage?
		requires pkt.msg.m.CShard?
		requires CHostIsValid(s)
		requires CPacketIsValid(pkt)
		...
		// requires pkt.msg.m.recipient in s.constants.hostIds
		ensures CHostIsValid(s')
		ensures CSingleMessageIsValid(sm)
		ensures SeqIsValid(out, CPacketIsValid)
		ensures SeqIsAbstractable(out, AbstractifyCPacketToShtPacket)
		ensures NextShard_Wrapper(
					AbstractifyCHostToHost(s),
					AbstractifyCHostToHost(s'),
					AbstractifyCPacketToShtPacket(pkt),
					SeqToSet(AbstractifySeq(out, AbstractifyCPacketToShtPacket))
...
method {:timeLimitMultiplier 2} HostModelNextReceiveMessage(host:CHost, cpacket:CPacket)  returns (host':CHost, sent_packets:seq<CPacket>)
	...
	requires cpacket.msg.m.CDelegate? ==> !EmptyKeyRange(cpacket.msg.m.range);
