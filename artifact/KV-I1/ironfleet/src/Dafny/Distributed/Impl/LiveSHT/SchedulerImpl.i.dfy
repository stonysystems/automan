...
class SchedulerImpl
{
    ...
    method ConstructUdpClient(constants:CConstants, me:EndPoint, ghost env_:HostEnvironment) returns (ok:bool, client:UdpClient?)
    ...
    method {:timeLimitMultiplier 2} Host_Init_Impl(constants:CConstants, me:EndPoint, ghost env_:HostEnvironment) returns (ok:bool)
        requires env_.Valid() && env_.ok.ok();
        requires CConstantsIsValid(constants);
        requires EndPointIsAbstractable(me);
            ...
            && host.constants == constants;
    {
        ...
        if (ok)
        {
            ...
            host := CHost_Init(me, constants.rootIdentity, constants.hostIds, constants.params);
            // host := CHost_Init(constants, me);
            nextActionIndex := 0;
            resendCount := 0;
            localAddr := host.me;
            Repr := { this } + UdpClientRepr(udpClient);
    ...
    method{:timeLimitMultiplier 8} HostNextReceivePacket(ghost udpEventLogOld:seq<UdpEvent>, rr:ReceiveResult, ghost receive_event:UdpEvent) returns (ok:bool, ghost udpEventLog:seq<UdpEvent>, ghost ios:seq<LSHTIo>)
        ...
        requires CPacketIsValid(rr.cpacket);
            ...
            )
    {
        ...
        assert Valid();
        assert ReceivePacket(old(AbstractifyToHost()), AbstractifyToHost(), AbstractifyCPacketToShtPacket(cpacket), SeqToSet(AbstractifySeq(sent_packets, AbstractifyCPacketToShtPacket)), AbstractifyCPacketToShtPacket(ack));
        ...
        assert SeqToSet(AbstractifySeq(sent_packets, AbstractifyCPacketToShtPacket)) == ExtractPacketsFromLSHTPackets(ExtractSentPacketsFromIos(ios));
        assert ReceivePacket_Wrapper(old(AbstractifyToHost()), AbstractifyToHost(), AbstractifyCPacketToShtPacket(cpacket), SeqToSet(AbstractifySeq(sent_packets, AbstractifyCPacketToShtPacket)));
...
//                assert LHost_ReceivePacketWithoutReadingClock(old(AbstractifyToHost()), AbstractifyToHost(), ios);
    ...
    lemma {:axiom} lemma_CSingleMessage(host:CHost, msg:CSingleMessage)
        requires msg.CSingleMessage?
        requires CHostIsValid(host)
        ensures msg.m.CDelegate? ==> !EmptyKeyRange(msg.m.range)
        // ensures msg.m.CGetRequest? ==> msg.m.k_getrequest in host.delegationMap
        // ensures msg.m.CSetRequest? ==> msg.m.k_setrequest in host.delegationMap
        // ensures msg.m.CShard? ==> CDelegateForKeyRangeIsHost(host.delegationMap, msg.m.kr, host.me) && msg.m.recipient != host.me && msg.m.recipient in host.constants.hostIds 
            ...
            && old(Env().udp.history()) + udpEventLog == Env().udp.history());
    {
        ...
        lemma_Host_ProcessReceivedPacket_Next(ok,udpEventLog,ios);
        ...
        //             assert Process_Message(old(AbstractifyToHost()), AbstractifyToHost(), SeqToSet(AbstractifySeq(sent_packets, AbstractifyCPacketToShtPacket)))
        //                 || HostIgnoringUnParseable(old(AbstractifyToHost()), AbstractifyToHost(), SeqToSet(AbstractifySeq(sent_packets, AbstractifyCPacketToShtPacket)));
        //         } else {
        //             assert false;
        //         }
        ...
        //         if HostIgnoringUnParseable(old(AbstractifyToHost()), AbstractifyToHost(), SeqToSet(AbstractifySeq(sent_packets, AbstractifyCPacketToShtPacket))) {
    ...
    lemma {:axiom} lemma_Host_ProcessReceivedPacket_Next(ok:bool, udpEventLog:seq<UdpEvent>, ios:seq<LSHTIo>)
        // requires nextActionIndex == 1;
        // requires Valid();
        ensures Repr == old(Repr);
        ensures Env() == old(Env());
