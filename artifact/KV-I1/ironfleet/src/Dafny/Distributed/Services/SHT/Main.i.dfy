...
module Main_i refines Main_s {
    ...
    function AbstractifyConcretePacket(p:LPacket<EndPoint,seq<byte>>) : LPacket<NodeIdentity, SingleMessage>
    ...
    function AbstractifyConcreteEnvStep(step:LEnvStep<EndPoint,seq<byte>>) : LEnvStep<NodeIdentity, SingleMessage>
    ...
    function AbstractifyConcreteSentPackets(sent:set<LPacket<EndPoint,seq<byte>>>) : set<LPacket<NodeIdentity, SingleMessage>>
        requires forall p :: p in sent ==> LPacketIsAbstractable(p);
    {
        set p | p in sent :: AbstractifyConcretePacket(p)
    }

    function AbstractifyConcreteEnvironment(ds_env:LEnvironment<EndPoint,seq<byte>>) : LEnvironment<NodeIdentity, SingleMessage>
    ...
    function AbstractifyConcreteConfiguration(ds_config:ConcreteConfiguration) : SHTConfiguration
        requires CConstantsIsValid(ds_config);
    {
        AbstractifyCSHTConfigurationToSHTConfiguration( 
                                CSHTConfiguration(
                                                        ds_config.hostIds,
    ...
    lemma lemma_IosRelations(ios:seq<LIoOp<EndPoint, seq<byte>>>, r_ios:seq<LIoOp<NodeIdentity, SingleMessage>>)
        returns (sends:set<LPacket<EndPoint, seq<byte>>>, r_sends:set<LPacket<NodeIdentity, SingleMessage>>) 
    ...
    lemma lemma_IsValidEnvStep(de:LEnvironment<EndPoint, seq<byte>>, le:LEnvironment<NodeIdentity, SingleMessage>)
    ...
    lemma lemma_LEnvironmentNextHost(de :LEnvironment<EndPoint, seq<byte>>, le :LEnvironment<NodeIdentity, SingleMessage>,
                                      de':LEnvironment<EndPoint, seq<byte>>, le':LEnvironment<NodeIdentity, SingleMessage>)
        requires ConcreteEnvironmentIsAbstractable(de);
        requires ConcreteEnvironmentIsAbstractable(de');
        requires AbstractifyConcreteEnvironment(de)  == le;
        requires AbstractifyConcreteEnvironment(de') == le';
    ...
    lemma {:axiom} lemma_lemma_AllConfigConsistent(config:ConcreteConfiguration, db:seq<DS_State>, i:int, s:LSHT_State)
        requires IsValidBehavior(config, db);
        requires 0 <= i < |db|;
        ...
        ensures  forall k :: 0 <= k < |s.config.hostIds| ==> s.hosts[k].host.me == s.config.hostIds[k];
    {
        if i == 0
        {
            ...
            calc {
                ...
                |AbstractifyCConstantsToConstants(db[0].config).hostIds|;
                |ls.config.hostIds|;
            }
            var shtconcreteconfig := CSHTConfiguration(
                                                        config.hostIds,
                                                        config.rootIdentity,
                                                        config.params
                                                        );
            assert CSHTConfigurationIsAbstractable(shtconcreteconfig)
            ...
            // assert WFSHTConfiguration(AbstractifyCSHTConfigurationToConfiguration(shtconcreteconfig));
            assert AbstractifyCSHTConfigurationToSHTConfiguration(shtconcreteconfig) == s.config;
            forall i | 0 <= i < |ls.config.hostIds|
                ensures ls.hosts[i].host.me == ls.config.hostIds[i];
            {
                reveal_SeqIsUnique();
            }
            lemma_lemma_AllConfigConsistent(config,db,i,s);
            return;
        }
        lemma_DsConsistency(config, db, i-1);
        lemma_DsConsistency(config, db, i);
        ...
        ensures  LSHT_Next(ls, ls');
    {
        ...
        {
            lemma_ReplicasDistinct(ls.config.hostIds, index, other_idx);
    ...
    lemma {:axiom} lemma_ReplicasDistinct(replica_ids:seq<NodeIdentity>, i:int, j:int)
        ensures ReplicasDistinct(replica_ids,i,j)
   
    lemma {:axiom} lemma_RefinementToLiveSHTProof(config:ConcreteConfiguration, db:seq<DS_State>, sb:seq<LSHT_State>)
        requires |db| > 0;
...
//                    InvHolds(c, sht_states);
        ...
        lemma_RefinementToLiveSHTProof(config,db,sb);
    }
    ...
    lemma {:axiom} lemma_RefinementProofForFixedBehavior(config:ConcreteConfiguration, db:seq<DS_State>, sb:seq<ServiceState>)
        requires |db| > 0;
        requires DS_Init(db[0], config);
        requires forall i {:trigger DS_Next(db[i], db[i+1])} :: 0 <= i < |db| - 1 ==> DS_Next(db[i], db[i+1]);
        ...
        ensures  forall i {:trigger Service_Correspondence(db[i].environment.sentPackets, sb[i])} :: 0 <= i < |db| ==> Service_Correspondence(db[i].environment.sentPackets, sb[i]);
    {
        ...
        //     }
        // }
        lemma_RefinementProofForFixedBehavior(config, db, sb);
    }
