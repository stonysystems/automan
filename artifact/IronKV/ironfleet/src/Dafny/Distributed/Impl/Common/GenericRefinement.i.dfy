  ...
  reads f.reads
{
  ...
  /*
  predicate SeqIsValid<T(!new), CT(!new)>(s:seq<CT>, ValidateValue:CT~>T)
    reads ValidateValue.reads
  {
    && (forall i :: 0 <= i < |s| ==> ValidateValue.requires(s[i]))
  ...
  */
  ...
  //   reads ValidateValue.reads
  // {
  //   && (forall i :: 0 <= i < |s| ==> ValidateValue.requires(s[i]) && ValidateValue(s[i]) )
