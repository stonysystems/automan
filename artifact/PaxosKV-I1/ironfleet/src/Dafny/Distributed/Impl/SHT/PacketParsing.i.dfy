...
{
    var cm := if val.c == 0 then
        CSingleMessage(val.val.t[0].u as nat, parse_EndPoint(val.val.t[1]), parse_Message(val.val.t[2]))
    else
        CAck(val.val.u as nat);
    lemma_parse_CSingleMessage(val, cm);
    cm
}

lemma {:axiom} lemma_parse_CSingleMessage(val:V, cm:CSingleMessage)
    requires ValInGrammar(val, CSingleMessage_grammar());
    ensures  CSingleMessageIsAbstractable(cm);
    ensures  CSingleMessageIsValid(cm);
    ensures  ValidVal(val) ==> CSingleMessageIs64Bit(cm); 
    ...
    ensures  0 <= SizeOfV(val) < 16 + max_val_len();
{
    ...
    lemma_MarshallOptionalValue(c, val);
}

lemma {:axiom} lemma_MarshallOptionalValue(c:OptionalValue, val:V)
    requires ValidOptionalValue(c);
    ensures  ValInGrammar(val, OptionalValue_grammar());
    ensures  ValidVal(val);
    ensures  parse_OptionalValue(val) == c;
