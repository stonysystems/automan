...
method {:fuel CReplicaIsValid,0,0} {:timeLimitMultiplier 2} ReplicaNextProcessPacketRequest(
  ...
  requires r.Valid()
  requires old_net_history + [receive_event] == r.Env().net.history() // records all new messages have received
  requires CConfigurationIsValid(r.replica.constants.all.config)
  ...
  requires Replica_Next_Process_Request_Preconditions(r.replica, cpacket) // packet and replicastate are valid
...
method {:fuel AbstractifyCReplicaToLReplica,0,0} {:fuel CReplicaIsValid,0,0} {:timeLimitMultiplier 5} ReplicaNextProcessPacketAppStateSupply(
  r:ReplicaImpl,
  cpacket:CPacket,
  ghost old_net_history:seq<NetEvent>,
  ghost receive_event:NetEvent,
            ...
            && old_net_history + netEventLog == r.Env().net.history()
{
  ...
  r.replica, sent_packets, replicaChanged := CReplicaNextProcessAppStateSupply(r.replica, cpacket);
  // if replicaChanged {
  ...
  lemma_DeliverPackets(r, sent_packets);
  ghost var send_events, send_ios;
  assert r.Valid();
  ok, send_events, send_ios := DeliverOutboundPackets(r, sent_packets);
  if (!ok) { return; }
  ...
  lemma_EstablishQLReplicaNextProcessPacketWithoutReadingClock(replica_old, AbstractifyCReplicaToLReplica(r.replica),
                                                               lpacket, AbstractifyOutboundCPacketsToSeqOfRslPackets(sent_packets), ios);
  // //print ("Replica_Next_ProcessPacketWithoutReadingClock_body: Exit\n");
  lemma_ReplicaNextProcessPacketAppStateSupply(r, cpacket, old_net_history, receive_event, receive_io, ok, netEventLog, ios);
}

lemma {:axiom} lemma_DeliverPackets(r:ReplicaImpl, packets:OutboundPackets)
  ensures r.Valid()
  ensures OutboundPacketsIsValid(packets)
  ensures OutboundPacketsIsAbstractable(packets)
  ensures OutboundPacketsHasCorrectSrc(packets, r.replica.constants.all.config.replica_ids[r.replica.constants.my_index]);

lemma {:axiom} lemma_ReplicaNextProcessPacketAppStateSupply(r:ReplicaImpl,
  cpacket:CPacket,
   old_net_history:seq<NetEvent>,
   receive_event:NetEvent,
   receive_io:RslIo,
            ...
            && old_net_history + netEventLog == r.Env().net.history()
{
  if (cpacket.msg.CMessage_Invalid?) {
    ...
    // lemma_Replica_Next_ProcessPacketWithoutReadingClock_body(r, old_net_history, ok, netEventLog, ios);
  } else {
    assert false;
  }
  // lemma_Replica_Next_ProcessPacketWithoutReadingClock_body(r, old_net_history, ok, netEventLog, ios);
...
lemma {:axiom} lemma_Replica_Next_ProcessPacketWithoutReadingClock_body(r:ReplicaImpl,
  // cpacket:CPacket,
   old_net_history:seq<NetEvent>,
  //  receive_event:NetEvent,
  //  receive_io:RslIo,
