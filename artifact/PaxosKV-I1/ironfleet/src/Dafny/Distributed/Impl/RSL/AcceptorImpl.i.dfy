  ...
  /** 7 lines manual code for I1 */
    ...
    // var t4 := 
      if s.log_truncation_point <= inp.msg.opn_2a then 
        (CAcceptor(s.constants, m.bal_2a, CAddVoteAndRemoveOldOnes(s.votes, m.opn_2a, CVote(m.bal_2a, m.val_2a), newLogTruncationPoint), s.last_checkpointed_operation, newLogTruncationPoint), t1)
        // t1 
      else 
        (CAcceptor(s.constants, m.bal_2a, s.votes, s.last_checkpointed_operation, newLogTruncationPoint), t1)
        // var t1 := s.votes; 
        // t1; 
    //   var t5 := s.constants; 
      ...
      if inp.src in s.constants.all.config.replica_ids then 
        ...
        // var t1 := 
          if 0 <= sender_index && sender_index < |s.last_checkpointed_operation| && inp.msg.opn_ckpt > s.last_checkpointed_operation[sender_index] then 
            ...
            CAcceptor(s.constants, s.max_bal, s.votes, s.last_checkpointed_operation[sender_index := inp.msg.opn_ckpt], s.log_truncation_point) 
          else 
            s
            // t1; 
          // t1; 
