...
function sht_cmd_line_parsing(args:seq<seq<byte>>) : CConstants
{
...
method parse_cmd_line(id:EndPoint, args:seq<seq<byte>>) returns (ok:bool, config:CConstants, my_index:uint64)
                ...
                && config.hostIds[my_index] == id
{
  ...
  config := CConstants(root_identity, endpoints, StaticParams());
}
}
