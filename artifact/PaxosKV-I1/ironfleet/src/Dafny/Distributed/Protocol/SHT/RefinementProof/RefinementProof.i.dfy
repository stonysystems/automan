...
lemma {:timeLimitMultiplier 2} SetPreservesRefinement_HostClaims(s:SHT_State, s':SHT_State, id:NodeIdentity, recv:set<Packet>, out:set<Packet>, rpkt:Packet, ko:Key, sm:SingleMessage, m:Message)
    requires HiddenInv(s) && InvBasics(s);
    requires HiddenInv(s') && InvBasics(s');
    requires SHT_Next(s, s');
    requires SHT_NextPred(s, s', id, recv, out);
...
lemma SetPreservesRefinement_ko(s:SHT_State, s':SHT_State, id:NodeIdentity, recv:set<Packet>, out:set<Packet>, rpkt:Packet, ko:Key, sm:SingleMessage, m:Message)
    requires HiddenInv(s) && InvBasics(s);
    requires HiddenInv(s') && InvBasics(s');
    requires SHT_Next(s, s');
    requires SHT_NextPred(s, s', id, recv, out);
...
lemma SetPreservesRefinement_main(s:SHT_State, s':SHT_State, id:NodeIdentity, recv:set<Packet>, out:set<Packet>, rpkt:Packet, sm:SingleMessage, m:Message)
    requires HiddenInv(s) && InvBasics(s);
    requires HiddenInv(s') && InvBasics(s');
    requires SHT_Next(s, s');
    requires SHT_NextPred(s, s', id, recv, out);
...
lemma NextShardPreservesRefinement_k(s:SHT_State, s':SHT_State, id:NodeIdentity, recv:set<Packet>, out:set<Packet>, kr:KeyRange, recipient:NodeIdentity, sm:SingleMessage, k:Key, shouldSend:bool)
    requires HiddenInv(s) && InvBasics(s);
    requires HiddenInv(s') && InvBasics(s');
    requires SHT_Next(s, s');
    requires SHT_NextPred(s, s', id, recv, out);
...
// triggers.
lemma NextShardPreservesRefinement_JustAgreement(s:SHT_State, s':SHT_State, id:NodeIdentity, recv:set<Packet>, out:set<Packet>, kr:KeyRange, recipient:NodeIdentity, sm:SingleMessage, shouldSend:bool)
    requires HiddenInv(s) && InvBasics(s);
    requires HiddenInv(s') && InvBasics(s');
    requires NoConfigChanged(s, s');
    requires id in AllHostIdentities(s);
...
lemma NextShardPreservesRefinement_main(s:SHT_State, s':SHT_State, id:NodeIdentity, recv:set<Packet>, out:set<Packet>, kr:KeyRange, recipient:NodeIdentity, sm:SingleMessage, shouldSend:bool)
    requires HiddenInv(s) && InvBasics(s);
    requires HiddenInv(s') && InvBasics(s');
    requires id in AllHostIdentities(s);
    requires HiddenSHT_Next_and_SHT_NextPred(s, s', id, recv, out);
...
{ 
    ...
    if NextGetRequest(h, h', rpkt, out) {
        ...
        //assume false;
        var sm:SingleMessage,m,b :| NextGetRequest_Reply(h, h', rpkt.src, rpkt.msg.seqno, k, sm, m, out, b);
        if(owner == h.me && b) {
        ...
        var sm:SingleMessage,m,b :| NextSetRequest_Complete(h, h', rpkt.src, rpkt.msg.seqno, rpkt.msg.m, sm, m, out, b);
    ...
    //assume false;
        ...
        if (   rpkt.msg.m.recipient == h.me 
            ...
            //assume false;
            lemma_NextShard_Wrapper(s, s', id, recv, out);
...
lemma {:axiom} lemma_NextShard_Wrapper(s:SHT_State, s':SHT_State, id:NodeIdentity, recv:set<Packet>, out:set<Packet>)
    requires MapComplete(s');
    requires SHT_Next(s, s');
    requires SHT_NextPred(s, s', id, recv, out);
    ensures s'.hosts[id] == s.hosts[id].(receivedPacket := None);
