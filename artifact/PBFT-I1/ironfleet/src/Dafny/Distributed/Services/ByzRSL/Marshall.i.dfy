...
include "../../Impl/LiveByzRSL/PacketParsing.i.dfy"
...
module MarshallProof_i {
    ...
    ensures  msg.reply == reply
  {
    ...
    if 0 <= seqno < 0x1_0000_0000_0000_0000 {
      assert marshalled_bytes == [ 0, 0, 0, 0, 0, 0, 0, 7] + AbstractServiceRSL_s.Uint64ToBytes(seqno as uint64) + MarshallAppMessage(reply);
      var cmsg := PaxosDemarshallData(bytes);
      var data := bytes;
      var v := DemarshallFunc(data, g);
      ...
      // Prove that the first 8 bytes are correct
      assert msgCaseId == SeqByteToUint64(bytes[..8]) == 7;
