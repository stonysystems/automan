...
function {:opaque} GetSequenceOfRequestBatches(qs:seq<QuorumOf2bs>):seq<RequestBatch>
  ...
  ensures  !exists q :: IsValidQuorumOf2bs(b[i], q) && q.opn == bound
{
  ...
  if exists q :: IsValidQuorumOf2bs(b[i], q) && q.opn == bound
  {
    ...
    assert |q.indices| >= LMinQuorumSize(b[i].constants.config);
    assert LMinQuorumSize(b[i].constants.config) >= 1;
    assert |q.indices| > 0;
