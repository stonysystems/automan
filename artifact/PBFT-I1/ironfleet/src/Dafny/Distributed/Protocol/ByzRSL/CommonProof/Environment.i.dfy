...
lemma lemma_PacketSeqStaysInSentPackets(
  b:Behavior<RslState>,
  c:LConstants,
  i:int,
  j:int,
  ...
  requires IsValidBehaviorPrefix(b, c, j)
  requires 0 <= i <= j
  requires forall p :: p in packets ==> p in b[i].environment.sentPackets
  ensures  forall p :: p in packets ==> p in b[j].environment.sentPackets
{
  forall p | p in packets
    ensures p in b[j].environment.sentPackets
  {
