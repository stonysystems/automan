...
lemma {:opaque} lemma_AppStateAlwaysValid(
  b:Behavior<RslState>,
  c:LConstants,
  i:int,
  idx:int
  ...
  decreases i
{
  ...
  // assert IsValidQuorumOf2bsSequence(b[i-1], prev_qs);
  // assert |prev_qs| == b[i-1].replicas[idx].replica.executor.ops_complete;
  // assert b[i-1].replicas[idx].replica.executor.app == GetAppStateFromRequestBatches(GetSequenceOfRequestBatches(prev_qs));
  var q := lemma_DecidedOperationWasChosen(b, c, i-1, idx);
  assert IsValidQuorumOf2bs(b[i-1], q);
  qs := prev_qs + [q];
  reveal_GetSequenceOfRequestBatches();
  assert GetSequenceOfRequestBatches(qs) == GetSequenceOfRequestBatches(prev_qs) + [q.v];
  // assert  IsValidQuorumOf2bsSequence(b[i], qs);
  // assert  |qs| == b[i].replicas[idx].replica.executor.ops_complete;
  // assert  b[i].replicas[idx].replica.executor.app == GetAppStateFromRequestBatches(GetSequenceOfRequestBatches(qs));
}
  ...
  decreases i
{
  ...
  {
    // var ios := lemma_ActionThatChangesReplicaIsThatReplicasAction(b, c, i-1, idx);
    if ios[0].r.msg.RslMessage_Request? 
    {
    ...
    // assert false;
    assert ios[0].r.msg.RslMessage_2av?;
  }
