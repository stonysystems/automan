...
import opened GenericRefinement_i
    ...
    function method CConvert1bPacketsSeqToMsgSeq(S: seq<CPacket>) : seq<CMessage> 
		...
		ensures 
            var lr := Convert1bPacketsSeqToMsgSeq(AbstractifySeq(S, AbstractifyCPacketToRslPacket)); 
            var cr := CConvert1bPacketsSeqToMsgSeq(S); 
            (forall i :: i in cr ==> CMessageIsValid(i)) 
            && (AbstractifySeq(cr, AbstractifyCMessageToRslMessage)) == (lr)
	{
		if |S| == 0 then 
			[] 
		else 
