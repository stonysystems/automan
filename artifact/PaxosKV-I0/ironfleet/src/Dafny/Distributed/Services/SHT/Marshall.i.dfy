...
module MarshallProof_i {
    ...
    lemma {:fuel ValInGrammar,3} lemma_ParseMarshallGetRequest(bytes:seq<byte>, msg:SingleMessage)
        ...
        ensures  bytes == MarshallServiceGetRequest(AppGetRequest(msg.seqno, msg.m.k_getrequest), reserved_bytes);
    {
        ...
        assert v.c == 0;
        assert cs == CSingleMessage(v.val.t[0].u as nat, parse_EndPoint(v.val.t[1]), parse_Message(v.val.t[2]));
    ...
    lemma {:fuel ValInGrammar,5} {:timeLimitMultiplier 2} lemma_ParseMarshallSetRequest(bytes:seq<byte>, msg:SingleMessage)
        returns (reserved_bytes:seq<byte>)
    ...
    lemma {:fuel ValInGrammar,5} {:timeLimitMultiplier 2} lemma_ParseMarshallReply(
        bytes:seq<byte>, reply:AppReply, msg:SingleMessage, reserved_bytes:seq<byte>) 
        requires CSingleMessageIsAbstractable(SHTDemarshallData(bytes));
        requires AbstractifyCSingleMessageToSingleMessage(SHTDemarshallData(bytes)) == msg;
