...
method{:timeLimitMultiplier 2} Receive(netClient:NetClient, localAddr:EndPoint, config:CConfiguration, msg_grammar:G) returns (rr:ReceiveResult, ghost netEvent:NetEvent)
            ...
            // && PaxosEndPointIsValid(rr.cpacket.src, config)
{
  ...
  if cmessage.CMessage_Invalid? {
    ...
    // print cpacket.msg.seqno, " request time:" , time, "\n";
                 ...
                 && SendLogReflectsPacket(netEventLog, packets.p)
{
  ...
  if opt_packet.None? {
    ...
    if cpacket.msg.CMessage_Reply?
    {
      var time := Time.GetDebugTimeTicks();
      // print("NetRSL reply bal:", cpacket.msg.seqno_reply, "\n");
      print cpacket.msg.seqno_reply, "reply time:", time, "\n";
    }
    // print("NetRSL: Send Packet\n");
    ok := netClient.Send(dstEp.public_key, buffer);
    if (!ok) { 
      // print("NetRSL: send packet fail\n"); 
      return; }
