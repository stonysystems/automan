...
{
  var e := EndPoint(val.b);
  lemma_parse_EndPoint(val, e);
  e
}

lemma {:axiom} lemma_parse_EndPoint(val:V, e:EndPoint)
  requires ValInGrammar(val, EndPoint_grammar())
  ensures  EndPointIsAbstractable(e)
  ...
  ensures  CRequestIsAbstractable(parse_Request(val))
{
  ...
  CRequest(ep, val.t[1].u as int, val.t[2].b)
}
  ...
  ensures  CReplyIsAbstractable(parse_Reply(val))
{
  ...
  var ep := parse_EndPoint(val.t[0]);
  CReply(ep, val.t[1].u as int, val.t[2].b)
}
  ...
  ensures  ValidVal(val) ==> CMessageIs64Bit(parse_Message_Request(val))
{
  ...
  CMessage_Request(val.t[0].u as int, val.t[1].b)
}
  ...
  ensures  ValidVal(val) ==> CMessageIs64Bit(parse_Message_Reply(val))
{
  ...
  CMessage_Reply(val.t[0].u as int, val.t[1].b)
}
...
{
  && msg.CMessage_2a?
  && CRequestBatchIsValid(msg.val_2a)
}
...
method DetermineIfValidVote(vote:CVote) returns (b:bool)
    ...
    ensures b == CVoteIsValid(vote)
  {
    ...
    lemma_DetermineIfValidVote(vote, b);
  }

lemma {:axiom} lemma_DetermineIfValidVote(vote:CVote, b:bool)
  requires CVoteIsAbstractable(vote)
    requires CVoteIs64Bit(vote)
    ensures b == CVoteIsValid(vote)
// method DetermineIfValidVote(vote:CVote) returns (b:bool)
  ...
  ensures  b == CVotesIsValid(votes)
{
  ...
  // lemma_DetermineIfValidVotes(votes, b);
}

lemma {:axiom} lemma_DetermineIfValidVotes(votes:CVotes, b:bool)
  requires CVotesIsAbstractable(votes)
  requires CVotesIs64Bit(votes)
  ensures  b == CVotesIsValid(votes)
  ...
  ensures  b == CRequestBatchIsValid(c)
{
  ...
  if !b {
    // lemma_DetermineIfValidRequestBatch(c, b);
    return;
  }
  ...
  {
    // if !CRequestValid(c[pos]) {
    //   assert !CRequestValid(c[pos]);
    //   b := false;
    //   return;
    // }

    var a := c[pos];
    if !CAppRequestMarshallable(a.request) || !EndPointIsValidPublicKey(a.client) {
...
//   assert parse_Message_Request(val) == c;
// }
...
//   }
// }
  predicate method Msg2aInRange(c:CMessage)
    requires c.CMessage_2a?
  {
    && BallotInRange(c.bal_2a)
    && Uint64InRange(c.opn_2a)
       // && VotesInRange(c.votes)
  }
...
method MarshallMessage_2a(c:CMessage) returns (val:V)
    ...
    ensures  Msg2aInRange(c) ==> ValInGrammar(val, CMessage_2a_grammar())
    ensures  Msg2aInRange(c) ==> ValidVal(val)
    ensures  Msg2aInRange(c) ==> parse_Message_2a(val) == c
  {
    if Msg2aInRange(c){
...
lemma {:axiom} lemma_MarshallMessage(c:CMessage, val:V)
  requires Marshallable(c)
  ...
  ensures ValidVal(val)
{
    ...
    ensures ValidVal(v)
  {
    ...
    assert CRequestIsValid(c.max_val[i]);
...
lemma {:axiom} lemma_MarshallableBound(c:CMessage, val:V)
  requires Marshallable(c)
  requires ValInGrammar(val, CMessage_grammar())
  requires ValidVal(val)
...
//       <= { lemma_mul_is_commutative(|val.val.t[2].a|, 0xffff_ffff_ffff_ffff);
//           lemma_mul_is_commutative(|val.val.t[2].a|, 0x10_0018 + MaxAppRequestSize());
//           lemma_mul_inequality(|val.val.t[2].a|, 100, 0x10_0018 + MaxAppRequestSize());
//         }
//       // 8 + (0x10_0018 + MaxAppRequestSize())*RequestBatchSizeLimit();
//       8 + (0x10_0018 + MaxAppRequestSize())*100;
//     }
//   } else if c.CMessage_2b? {
//     lemma_SeqSum3(val.val);
...
//       <=
//       8 + (0x10_0018 + MaxAppRequestSize())*|val.val.t[2].a|;
//       <= { lemma_mul_is_commutative(|val.val.t[2].a|, 100);
//           lemma_mul_is_commutative(|val.val.t[2].a|, 0x10_0018 + MaxAppRequestSize());
//           lemma_mul_inequality(|val.val.t[2].a|, 100, 0x10_0018 + MaxAppRequestSize());
//         }
//       8 + (0x10_0018 + MaxAppRequestSize())*100;
