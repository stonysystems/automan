...
datatype CMessage =
    CMessage_Invalid()
  | CMessage_Request(seqno_req:int, val:CAppRequest)
  ...
  | CMessage_Reply(seqno_reply:int, reply:CAppReply)
  ...
  | CMessage_StartingPhase2(bal_2:CBallot, logTruncationPoint_2:COperationNumber)
	...
	predicate CMessageIsValid(s: CMessage) 
	{
		...
		case CMessage_Request(seqno_req, val) => CMessageIsAbstractable(s) && CAppRequestMarshallable(s.val)
		...
		case CMessage_Reply(seqno_reply, reply) => CMessageIsAbstractable(s) && CAppReplyMarshallable(s.reply)
		...
		case CMessage_AppStateSupply(bal_state_supply, opn_state_supply, app_state) => CMessageIsAbstractable(s) && CBallotIsValid(s.bal_state_supply) && COperationNumberIsValid(s.opn_state_supply) && CAppStateIsAbstractable(s.app_state)
	...
	predicate CMessageIsAbstractable(s: CMessage) 
	{
		...
		case CMessage_Request(seqno_req, val) => CAppRequestIsAbstractable(s.val)
		...
		case CMessage_Reply(seqno_reply, reply) => CAppReplyIsAbstractable(s.reply)
		...
		case CMessage_AppStateSupply(bal_state_supply, opn_state_supply, app_state) => CBallotIsAbstractable(s.bal_state_supply) && COperationNumberIsAbstractable(s.opn_state_supply) && CAppStateIsAbstractable(s.app_state)
		...
		requires CMessageIsAbstractable(s)
	{
		...
		case CMessage_Request(seqno_req, val) => RslMessage_Request(s.seqno_req, AbstractifyCAppRequestToAppRequest(s.val))
		...
		case CMessage_Reply(seqno_reply, reply) => RslMessage_Reply(s.seqno_reply, AbstractifyCAppReplyToAppReply(s.reply))
		...
		case CMessage_AppStateSupply(bal_state_supply, opn_state_supply, app_state) => RslMessage_AppStateSupply(AbstractifyCBallotToBallot(s.bal_state_supply), AbstractifyCOperationNumberToOperationNumber(s.opn_state_supply), AbstractifyCAppStateToAppState(s.app_state))
...
// function method CPacketValid(c:CPacket) : bool
// {
//   true
// }
...
// function method CBroadcastValid(c:CBroadcast) : bool
// {
//   true
// }
...
// function method OutboundPacketsValid(c:OutboundPackets) : bool
// {
//   true
// }
