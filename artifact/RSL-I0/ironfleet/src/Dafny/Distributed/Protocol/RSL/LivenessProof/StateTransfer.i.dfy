...
module LivenessProof__StateTransfer_i {
    ...
    ensures  BalLt(p_state_req.msg.bal_state_req, h.view)
  {
    ...
    lemma_NoReplicaBeyondViewImpliesNomax_balISent1aBeyondView(b, asp, step, h.view);
    ...
    lemma_max_balISent1aHasMeAsProposer(b, asp.c, step, primary_idx);

    if next_step <= h.start_step
    {
      lemma_max_balISent1aMonotonic(b, asp.c, next_step, h.start_step, primary_idx);
      assert false;
    }

    if next_step > h.start_step + 1
    {
      lemma_max_balISent1aMonotonic(b, asp.c, h.start_step + 1, step, primary_idx);
      assert false;
    }
    ...
    ensures  BalLeq(h.view, q.bal)
  {
    ...
    lemma_ConstantsAllConsistent(b, asp.c, i);
    lemma_Received1bPacketsAreFrommax_balISent1a(b, asp.c, h.start_step + 1, h.view.proposer_id);
