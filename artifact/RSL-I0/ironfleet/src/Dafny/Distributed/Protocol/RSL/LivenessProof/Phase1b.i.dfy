...
module LivenessProof__Phase1b_i {
                                          ...
                                          next(eventuallywithin(or(AcceptorHasMaxBalInViewTemporal(b, view, idx), not(NoReplicaBeyondViewTemporal(b, view))), processing_bound, PaxosTimeMap(b))))))
  {
      ...
      ensures sat(j, imply(x, next(eventuallywithin(or(y, z), t, f))))
    {
      if sat(j, x)
      {
        ...
        if !sat(k+1, or(y, z))
        {
          lemma_NoReplicaBeyondViewImpliesNomax_balISent1aBeyondView(b, asp, k+1, view);
                                             ...
                                             eventuallywithin(not(NoReplicaBeyondViewTemporal(b, view)), t, PaxosTimeMap(b))))))
  {
      ...
      ensures sat(j, imply(w, not(y)))
    {
      if sat(j, w) && sat(j, y)
      {
        ...
        lemma_MaxBalLeqmax_balPrimarySent1a(b, asp.c, j, idx);
      ...
      if sat(j, t)
      {
        ...
        else
        {
          ...
          if !sat(j+1, t)
          {
            ...
            lemma_max_balISent1aMonotonicOneStep(b, asp.c, j, primary_idx);
            ...
            lemma_max_balISent1aLeqView(b, asp, j+1, primary_idx);
            ...
            lemma_max_balISent1aLeqView(b, asp, j, primary_idx);
    ...
    decreases i
  {
    ...
    else
    {
      ...
      lemma_max_balISent1aHasMeAsProposer(b, asp.c, i, idx);
