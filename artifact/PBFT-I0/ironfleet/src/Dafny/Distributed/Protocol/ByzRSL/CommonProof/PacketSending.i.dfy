...
lemma lemma_ActionThatSends1cIsMaybeNominateValueAndSend1c(
  ps:RslState,
  ps':RslState,
  p:RslPacket
  ) returns (
  ...
  ensures  LIoOpSend(p) in ios
  ensures  SpontaneousIos(ios, 1)
  ensures  LReplicaNextReadClockMaybeNominateValueAndSend1c(ps.replicas[idx].replica, ps'.replicas[idx].replica,
                                                            SpontaneousClock(ios), ExtractSentPacketsFromIos(ios))
{
  assert ps.environment.nextStep.LEnvStepHostIos?;
  assert LIoOpSend(p) in ps.environment.nextStep.ios;
...
lemma lemma_ActionThatSends2avIsProcess1c(
  ps:RslState,
  ps':RslState,
  p:RslPacket
) returns (
...
lemma lemma_ActionThatSends2bIsMaybeSend2b(
  ps:RslState,
  ps':RslState,
  p:RslPacket
  ) returns (
