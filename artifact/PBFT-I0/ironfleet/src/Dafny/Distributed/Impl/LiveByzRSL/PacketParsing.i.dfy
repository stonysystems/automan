...
module LiveByzRSL__PacketParsing_i {
  ...
  */
  ...
  {
    && (forall i :: i in c ==> Uint64InRange(i) && VoteInRange(c[i]) && |c[i].max_val| <= RequestBatchSizeLimit())
  }
    ...
    ensures VotesInRange(c) ==> SeqSum(val.a) <= |c| * (8 + (8 + 8) + (8 + (16 + max_val_len())*RequestBatchSizeLimit()))
  {
    if VotesInRange(c) {
      if |c| == 0 {
        ...
        var b := |c| < max_votes_len();  // max_votes_len
        if !b {
          print "votes exceeds the max length";
          lemma_vote_len(c);
          return;
        ...
        // var b2 := forall i :: i in c ==> |c[i].max_val| <= RequestBatchSizeLimit();
        // if !b {
        //   print "RequestBatch exceeds size limit\n";
        //   return;
        // }
    ...
    ensures  Msg1bInRange(c) ==> 0 <= SizeOfV(val) < MaxPacketSize() - 8
  {
    if Msg1bInRange(c){
      ...
      var b := |c.votes| < max_votes_len();  // max_votes_len
      if !b {
        print "votes exceeds the max length";
  ...
  lemma {:timeLimitMultiplier 2} lemma_VoteValValid(c:CVote, val:V)
    ...
    ensures ValidVal(val)
  {
    var b := |c.max_val| <= RequestBatchSizeLimit();
    if !b {
      // print "RequestBatch exceeds size limit.\n";
      lemma_RequestQueue_len(c.max_val);
      return;
    }
    ...
    // ensures  !c.CMessage_1b? && !c.CMessage_AppStateSupply? ==> 0 <= SizeOfV(val) < MaxPacketSize()
  {
    ...
    if c.CMessage_Request? {
      ...
      */
      ...
      lemma_RequestQueue_len(c.val_1c);
      ...
      lemma_RequestQueue_len(c.val_2av);
      lemma_SeqSum3(val.val);
      ...
      lemma_RequestQueue_len(c.val_2b);
      lemma_SeqSum3(val.val);
