...
{
  b := (|votes.v| as uint64) < max_votes_len() as uint64;  // max_votes_len
  if !b {
    return;
  }
  ...
  ensures  0 <= SizeOfV(val) < MaxPacketSize() - 8
{
  ...
  calc {
    ...
    32 + (|c.votes.v| * (32 + (16 + 64) * RequestBatchSizeLimit()));
    32 + (|c.votes.v| * 80032);
    < { lemma_mul_strict_inequality(|c.votes.v|, max_votes_len(), 80032); }
