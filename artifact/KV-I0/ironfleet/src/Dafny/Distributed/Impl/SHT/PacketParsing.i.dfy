...
function method parse_Message_GetRequest(val:V) : CMessage
    ...
    ensures  CMessageIsValid(parse_Message_GetRequest(val));
{
...
function method parse_Message_SetRequest(val:V) : CMessage
    ...
    ensures  CMessageIsValid(parse_Message_SetRequest(val));
{
    var msg := CSetRequest(parse_Key(val.t[0]), parse_OptionalValue(val.t[1]));
    lemma_parse_Message(msg);
    msg
}

lemma {:axiom} lemma_parse_Message(msg:CMessage)
    ensures CMessageIsValid(msg);
...
function method parse_Message_Reply(val:V) : CMessage
    ...
    ensures  CMessageIsValid(parse_Message_Reply(val));
{
    var msg := CReply(parse_Key(val.t[0]), parse_OptionalValue(val.t[1]));
    lemma_parse_Message(msg);
    msg
}
...
function method parse_Message_Redirect(val:V) : CMessage
    ...
    ensures  CMessageIsValid(parse_Message_Redirect(val));
{
...
function method parse_Message_Shard(val:V) : CMessage
    ...
    ensures  CMessageIsValid(parse_Message_Shard(val));
{
...
function method parse_Message_Delegate(val:V) : CMessage
    ...
    ensures  CMessageIsValid(parse_Message_Delegate(val));
    ensures  parse_Message_Delegate(val).CDelegate?;
    ensures  ValidVal(val) ==> HashtableIs64Bit(parse_Message_Delegate(val).h);
{
    var msg := CDelegate(parse_KeyRange(val.t[0]), parse_Hashtable(val.t[1]));
    lemma_parse_Message(msg);
    msg
}
...
function method parse_Message(val:V) : CMessage
    ...
    ensures  CMessageIsValid(parse_Message(val));
...
function method {:fuel ValidVal,2} parse_CSingleMessage(val:V) : CSingleMessage
    ...
    ensures  CSingleMessageIsValid(parse_CSingleMessage(val));
    ensures  ValidVal(val) ==> CSingleMessageIs64Bit(parse_CSingleMessage(val)); 
{
    if val.c == 0 then
        CSingleMessage(val.val.t[0].u as nat, parse_EndPoint(val.val.t[1]), parse_Message(val.val.t[2]))
    else
        CAck(val.val.u as nat)
}
...
method SHTDemarshallDataMethod(data:array<byte>) returns (msg:CSingleMessage)
    ...
    ensures CSingleMessageIsValid(msg)
...
predicate method SingleMessageInRange(c:CSingleMessage)
    requires CSingleMessageMarshallable(c)
{
    if c.CSingleMessage? then
        Uint64InRange(c.seqno)
    else
        Uint64InRange(c.ack_seqno)
...
method MarshallCSingleMessage(c:CSingleMessage) returns (val:V)
    ...
    ensures  ValidVal(val);
    ensures  SingleMessageInRange(c) ==> parse_CSingleMessage(val) == c;
    ensures   0 <= SizeOfV(val) < MaxPacketSize();
{
    if c.CSingleMessage? {
        ...
        if 0 <= c.seqno < 0x10000000000000000 {
            ...
            print "Marshall CSingleMessage seqno overflow\n";
        }
        var tuple := VTuple([VUint64(seqno), ep, msg]);
        lemma_SeqSum_3(tuple);
        assert ValidVal(tuple);     // OBSERVE
        val := VCase(0, tuple);
    } else {
        var seqno := 0;
        if 0 <= c.ack_seqno < 0x10000000000000000 {
            seqno := c.ack_seqno as uint64;
        } else {
            seqno := 0xffff_ffff_ffff_ffff;
            print "Marshall CSingleMessage seqno overflow\n";
        }
        val := VCase(1, VUint64(seqno));
    }
}
    ...
    ensures BufferRefinementAgreesWithMessageRefinement(msg, data[..]);
{
    ...
    //     }
    // }
    lemma_SHTMarshall(msg,data);
}

lemma {:axiom} lemma_SHTMarshall(msg:CSingleMessage, data:array<byte>)
    requires CSingleMessageIsAbstractable(msg);
    requires CSingleMessageMarshallable(msg);
    ensures fresh(data);
    ensures UdpPacketBound(data[..]);
...
//     // && CSingleMessageIsValid(cpacket.msg, params) 
