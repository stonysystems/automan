...
function ConstantsStateNull () : CConstants 
{
    CConstants(EndPointNull(), [], StaticParams())
}

function sht_config_parsing(args:seq<seq<uint16>>) : CConstants
{
    if args != [] && |args[1..]| % 2 == 0 then 
        var (ok, endpoints) := parse_end_points(args[1..]);
        CConstants(if |endpoints| > 0 then endpoints[0] else EndPointNull(), // Root is the first endpoint in the list
                       endpoints,
                       StaticParams())
    else 
        ConstantsStateNull()
...
function sht_cmd_line_parsing(env:HostEnvironment) : (CConstants, EndPoint)
    reads env;
    reads env.constants;
{
...
method parse_cmd_line(ghost env:HostEnvironment) returns (ok:bool, config:CConstants, my_index:uint64)
    requires HostEnvironmentIsValid(env);
    ensures ok ==> CConstantsIsValid(config);
            ...
            ok ==> config == config' && config.hostIds[my_index] == my_ep';
{
    ...
    config := CConstants(root_identity, hosts, StaticParams());
