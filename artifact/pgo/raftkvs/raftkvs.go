...
var jumpTable = distsys.MakeMPCalJumpTable(
	...
	distsys.MPCalCriticalSection{
		...
		Body: func(iface distsys.ArchetypeInterface) error {
			...
			if tla.ModuleEqualsSymbol(condition100.ApplyFunction(tla.MakeString("type")), Put(iface)).AsBool() {
				...
				case 0:
					// startTime := time.Now()
					// iface.EnsureArchetypeResourceLocal("AClient.startTime", tla.MakeNumber(int32(startTime.UnixNano() / 1e6)))
	...
	distsys.MPCalCriticalSection{
		...
		Body: func(iface distsys.ArchetypeInterface) error {
    		...
    		// iface.EnsureArchetypeResourceLocal("AClient.endTime", tla.MakeNumber(int32(endTime.UnixNano() / 1e6)))

			// startTimeVal := iface.RequireArchetypeResource("AClient.startTime")
			// endTimeVal := iface.RequireArchetypeResource("AClient.endTime")

			// startTime := time.Unix(0, startTimeVal)
			// endTime := time.Unix(0, endTimeVal.AsNumber().Int64())
...
var AClient = distsys.MPCalArchetype{
	...
	PreAmble: func(iface distsys.ArchetypeInterface) {
		...
		// iface.EnsureArchetypeResourceLocal("AClient.startTime", tla.Value{}) // Add start time field
        // iface.EnsureArchetypeResourceLocal("AClient.endTime", tla.Value{})   // Add end time field
	},
}
