...
module LiveRSL__Acceptor_i {
  ...
  function method Compute(x: int): int {
    x * 2
  }

  method ComputeImpl(x: int) returns (y: int)
    ensures y == Compute(x)
  {
    y := x * 2;
  }
  ...
  */
  ...
  predicate LAcceptorInit(a:LAcceptor, c:LReplicaConstants)
  {
    ...
    && (forall idx {:trigger a.last_checkpointed_operation[idx]} :: 0 <= idx < |c.all.config.replica_ids| ==>
                                                                      a.last_checkpointed_operation[idx] == 0)
