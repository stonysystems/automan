...
module LiveRSL__ReplicaModel_i {
  ...
  /** 13 lines of manual code for I1 */
  method CReplicaNextProcessAppStateSupply(s: CReplica, received_packet: CPacket) returns (s':CReplica, sequential_sent_packets:OutboundPackets, replicaChanged:bool, reply_cache_mutable:MutableMap<EndPoint, CReply>)
		requires CReplicaIsValid(s)
		requires CPacketIsValid(received_packet)
  ...
  method CReplicaNextSpontaneousMaybeEnterPhase2(s: CReplica) returns (s':CReplica, sequential_sent_packets:OutboundPackets) 
		requires CReplicaIsValid(s)
    ...
    && Replica_Common_Postconditions_NoPacket(AbstractifyCReplicaToLReplica(s), s', sequential_sent_packets) /** Manually Added */
    && OutboundPacketsIsValid(sequential_sent_packets) 
    && OutboundPacketsHasCorrectSrc(sequential_sent_packets, s.constants.all.config.replica_ids[s.constants.my_index])
	...
	{
		var newproposer, sent := 
			CProposerMaybeEnterPhase2(s.proposer, s.acceptor.log_truncation_point); 		
		s' := 
			s.(proposer := newproposer); 
    sequential_sent_packets := sent;		
